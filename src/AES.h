...
class AES {
 ...
 private:
  ...
  /**
   * @brief Dada una clave inicial de tamaño (128, 192, 256) se generarán tantas
   *        claves como sean necesarias para poder utilizarlas en cada una de
   *        las rondas del algoritmo. Es una implementación del key scheduler
   *        oficial de AES.
   */
  void ExpandKey();

  /**
   * @brief Rota las filas designadas por el algoritmo.
   */
  void ShiftRows();

  /**
   * @brief Inversa de ShiftRows para la desencriptación.
   */
  void InverseShiftRows();

  /**
   * @brief Rota los elementos de un array dado un desplazamiento.
   ...
   *        constante definida por el estandar AES.
   */
  inline void MixColumns();

  /**
   * @brief Multiplica una columna por la matriz MDS circulante de AES. Todas
   *        las operaciones realizadas (sumas y multiplicaciones) han de estar
   *        dentro de el espacio de galois 2^8 (GF(2^8)), lo que permite que
   *        no haya overflow ni underflow. Esto implica que las sumas serán
   *        xor y las multiplicaciones serán el módulo polinomial
   *        (mutltiplicar los dos números como polinómios y hayar el módulo de
   *        la división respecto del polinomio irreducible). Esto permite
   *        añadir optimizaciones operando directamente sobre los bits con
   *        puertas lógicas y desplazamientos.
   * @ref:
   ...
   *        a la utilizada en MixColumns para la desencriptación.
   */
  inline void InverseMixColumns();

  /**
   * @brief Multiplica una columna por la matriz MDS circulate inversa de AES.
   ...
   * estado.
   * @param[in] iteration Ronda actual.
   */
  inline void AddRoundKey(int iteration);

  /**
   * @brief Dada una clave se computará la clave siguiente.
   */
  void ComputeRoundKey();

  /**
   * @brief Aplica xor a dos arrays y lo guarda en otro.
   * @param[out] a Array destino.
   * @param[in] b Array 1.
   * @param[in] c Array 2.
